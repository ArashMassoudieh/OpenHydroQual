...
inline
bool
sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X, const uword n_eigvals)
  {
  arma_extra_debug_sigprint();
    ...
    run_aupd(n_eigvals, p, true /* sym, not gen */, n, tol, resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork, info);
    ...
    char which[3] = "LM"; // We want the eigenvalues with the largest magnitude.
...
inline
bool
sp_auxlib::eigs_gen(Col< std::complex<T> >& eigval, Mat< std::complex<T> >& eigvec, const SpBase<T, T1>& X, const uword n_eigvals)
  {
  arma_extra_debug_sigprint();
    ...
    run_aupd(n_eigvals, p, false /* gen, not sym */, n, tol, resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork, info);
...
inline
bool
sp_auxlib::eigs_gen(Col< std::complex<T> >& eigval, Mat< std::complex<T> >& eigvec, const SpBase< std::complex<T>, T1>& X, const uword n_eigvals)
  {
  arma_extra_debug_sigprint();
...
sp_auxlib::run_aupd
  ...
  )
  {
    ...
    // Real work array used in the basic Arnoldi iteration for reverse
    // communication.
    workd.set_size(3 * n);
