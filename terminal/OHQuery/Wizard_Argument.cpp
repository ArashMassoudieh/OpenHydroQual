...
QString Wizard_Argument::Calc(QMap<QString, WizardParameter>* params)
{
    if (param_constant_expression == "api")
    {
        ...
        {
            CTimeSeries<double> relativehumiditydata = weatherretriever.RetriveTimeSeriesOpenMeteo(start_date, end_date, location, "relative_humidity_2m")/100.0;
            wizard_entity->GetWizardScript()->AppendTimeSeries(this->wizard_entity->Name()+"-"+QString::fromStdString(parameter),this->wizard_entity->Name() + QString::number(x_location) + "_" + QString::number(y_location) + ".csv");
            FileName = WorkingDirectory() + "/" + this->wizard_entity->Name() + "_" + QString::fromStdString(parameter) +"_" + QString::number(x_location) + "_" + QString::number(y_location) + ".csv";
