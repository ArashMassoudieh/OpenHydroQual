        ...
        if (!opt.asLibrary) {
            ...
            ac << banner
               ...
               << "\n"
                  ...
                  "const char* ohq_kernel_state_name(int i) { return " << cls << "_state_name(i); }\n"
                  "int    ohq_kernel_step(void* h) { return " << cls << "_step((" << cls << "_handle*)h); }\n"
                  "void   ohq_kernel_initialize_at(void* h, double tstart, double dt0) { " << cls << "_initialize_at((" << cls << "_handle*)h, tstart, dt0); }\n"
                  "double ohq_kernel_parameter(const void* h, int i) { return " << cls << "_parameter((const " << cls << "_handle*)h, i); }\n"
                  "/* G4: runtime forcing injection, addressed by (object, quantity) name */\n"
                  "int    ohq_kernel_n_series(void) { return " << cls << "_n_series(); }\n"
                  "const char* ohq_kernel_series_object(int k) { return " << cls << "_series_object(k); }\n"
                  "const char* ohq_kernel_series_quantity(int k) { return " << cls << "_series_quantity(k); }\n"
                  "int    ohq_kernel_set_series(void* h, const char* o, const char* q, const double* t, const double* v, int n)\n"
                  "       { return " << cls << "_set_series((" << cls << "_handle*)h, o, q, t, v, n); }\n"
                  "int    ohq_kernel_set_precipitation(void* h, const char* o, const char* q, const double* s, const double* e, const double* d, int n)\n"
                  "       { return " << cls << "_set_precipitation((" << cls << "_handle*)h, o, q, s, e, d, n); }\n"
                  ...
                  "       { " << cls << "_export_state((const " << cls << "_handle*)h, st, ma, li, lf); }\n"
                  "void   ohq_kernel_import_state(void* h, double t, const double* st, const double* ma, const int* li, const double* lf)\n"
                  "       { " << cls << "_import_state((" << cls << "_handle*)h, t, st, ma, li, lf); }\n"
                  "/* constituents: always present in the ABI; 0 / 0.0 when the model has none */\n"
               << (T ? "int    ohq_kernel_n_mass(void) { return " + cls + "_n_mass(); }\n"
                       "double ohq_kernel_mass(const void* h, int i) { return " + cls + "_mass((const " + cls + "_handle*)h, i); }\n"
                     : "int    ohq_kernel_n_mass(void) { return 0; }\n"
                       "double ohq_kernel_mass(const void* h, int i) { (void)h; (void)i; return 0.0; }\n")
