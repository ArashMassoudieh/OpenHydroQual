...
- **dt policy** mirrors the interpreter's Solve loop (`assign_D`/`interpol_D`
  ...
  2026-09-25, issues.md ISSUE 8), when the solver setting
  `timestep_clamp_series` = "All time series". The default, "Precipitation
  only", is the old behaviour in both: the generator then registers only
  `prec_timeseries` series with `addClampSeries`.
