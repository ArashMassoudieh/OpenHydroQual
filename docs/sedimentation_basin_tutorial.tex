...
$h$            & Depth of each layer                & $1$ m \\
$V$            & Volume of each layer               & $400$ m\textsuperscript{3} \\
...
\begin{table}[htbp]
\centering
\caption{Properties of the \code{Sed\_Basin} composite. The three interface
elevations and the floor elevation are the \emph{bottoms} of layers 1--4.}
\label{tab:basin}
\begin{tabular}{llr}
\toprule
Property & Meaning & Value \\
...
\menu{Elevation of the interface between layers 1 and 2} & bottom of layer 1 & $0$ m \\
\menu{Elevation of the interface between layers 2 and 3} & bottom of layer 2 & $-1$ m \\
\menu{Elevation of the interface between layers 3 and 4} & bottom of layer 3 & $-2$ m \\
\menu{Elevation of the clarifier floor}                  & bottom of layer 4 & $-3$ m \\
\menu{Initial volume of water in layer 1--4} & $V$ per layer & $400$ m\textsuperscript{3} each \\
\bottomrule
\end{tabular}
\end{table}
...
\item[The elevations must decrease downward.]
The settling term is gated on the elevation difference between the two ends of
each interface (Section~\ref{sec:physics}). If two adjacent layers were given
the same bottom elevation, the gate would close and \emph{no solids would settle
at all} --- with no error message. The layer volumes and the elevations are
independent inputs, so keep them consistent: $400$~m\textsuperscript{3} over
$400$~m\textsuperscript{2} is exactly the $1$~m of depth the elevations imply.
\end{description}

\screenshot[0.8]{03-basin-properties}{The property panel of the
\code{Clarifier (4 layers)} composite, showing every value from
Table~\ref{tab:basin} --- note \menu{Flow drawn downward through the layers}
$=0$. Scrolled off above are the per-constituent entries
(\code{P1:Concentration} and friends), which stay at zero: the basin starts
clean.}
...
create composite;type=Clarifier (4 layers),name=Sed_Basin,
    ...
    interface_elevation_1_2=0,interface_elevation_2_3=-1,
    interface_elevation_3_4=-2,floor_elevation=-3,
    layer1_storage=400,layer2_storage=400,
    layer3_storage=400,layer4_storage=400,
    x=700,y=900
...
bottom elevation and solids simply stop moving, with no diagnostic. If a model
shows zero removal, check the elevations first.
