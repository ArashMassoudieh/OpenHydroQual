...
Newton-Raphson convergence tolerance (relative) & \texttt{\small 0.\allowbreak{}001} \\
...
\subsection{Newton-Raphson convergence tolerance (relative)}
{\small \textbf{Default:} \texttt{0.\allowbreak{}001} \quad\textbullet\quad \textbf{Name in file:} \texttt{nr\_\allowbreak{}tolerance}}

Convergence criterion for the Newton iteration at each time step. It is a RELATIVE measure, not an absolute residual: the iteration stops once the residual has fallen to this fraction of its value at the start of the step, so 0.001 means the residual is reduced by three orders of magnitude and no further. Two other conditions also end the iteration -- an absolute residual floor of 1e-12, and an update that has become negligible against the solution norm. Because the criterion is relative and per step, the error left behind accumulates over a long run: on a model whose outputs are compared against measurements of similar magnitude, the default can leave a numerical error that is an appreciable fraction of the model-measurement residual, and it is worth tightening until the results stop changing. A smaller value does not necessarily cost time. A better converged step is a more stable one, so the solver can often take larger steps afterwards and finish sooner; measure rather than assume.
