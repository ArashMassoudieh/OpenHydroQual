% =====================================================================
%  OpenHydroQual -- general overview presentation
...
\begin{frame}{Every component's equations, in the application}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.62\textwidth}
      \figslotmini{S17}{S17_component_help.png}{0.99\columnwidth}{%
        Screenshot: Help, Component Description, showing the object type
        reference with governing equations.}
    \end{column}
    \begin{column}{0.35\textwidth}
      {\footnotesize \alert{Help $\rightarrow$ Component Description} lists
      every object type and, for each one, every property: its kind, its
      description, and \alert{the expression that governs it}.

      \vspace{0.6em}
      Nothing is hidden in compiled code. A leaky dam's flow is
      \texttt{under\_flow + over\_flow}, and both are there to read.}
      \vspace{0.6em}

      {\scriptsize Generated from the same JSON that defines the components, so
      the reference cannot drift out of step with the model.}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \begin{itemize}\setlength\itemsep{5pt}
        ...
        \item Full ASM1 and ASM implementations shipped as templates
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{itemize}\setlength\itemsep{5pt}
        ...
        \item Sorption, desorption and immobile phases
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{1.0em}
  {\footnotesize Transport is solved on the \alert{same network as flow}: one
  model, not a flow model handing results to a quality model. A reaction's rate
  expression is written in the property box like any other expression, as on
  the earlier slide.}
\end{frame}
...
\begin{frame}{Genetic-algorithm calibration}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \figslotmini{S19}{S19_column_agreement.png}{0.92\columnwidth}{%
        Figure: modelled against measured Cu for the column study.}
    \end{column}
    \begin{column}{0.50\textwidth}
      \textbf{\small A static calibration case}\\[3pt]
      {\footnotesize Copper sorption in stormwater biofiltration columns: sand
      against three amendments, biochar, activated carbon and zeolite, sampled
      at an upper and a lower port.}
      \vspace{0.5em}
      \begin{itemize}\setlength\itemsep{2pt}
        \item \alert{11 parameters} fitted at once: a sorption rate and a
              partition coefficient for each medium, plus porosity,
              dispersivity and the residual error term
        \item \alert{190 observations} across the columns
        \item Population-based global search, so no gradient and no
              differentiability requirement
      \end{itemize}
      \vspace{0.3em}
      {\footnotesize Pooled fit: \keyfact{$R^2 = 0.870$},
      RMSE $5.8\times10^{-3}$\,mg/L.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Bayesian parameter estimation}
  \centering
  \figslot{S18}{S18_column_posteriors.png}{\textwidth}{%
    Figure: marginal posterior densities against the priors, with the GA
    optimum marked.}
  \vspace{0.2em}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      {\scriptsize Marginal posteriors (filled) against the log-normal
      \alert{priors} they started from (dashed), with the \alert{GA optimum} as
      the gold diamond. 60{,}000 samples, 5{,}000 burn-in, split
      $\hat{R}$ 1.01--1.09.}
    \end{column}
    \begin{column}{0.48\textwidth}
      {\scriptsize Far narrower than the prior means the data
      \alert{identified} it: both sand terms, $K_d$ for biochar, and $\sigma$.
      Still prior-shaped means they did not: both \alert{zeolite} parameters
      are unconstrained here, which no best-fit number would reveal.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{The assimilation cycle}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.60\textwidth}
      \figslotmini{N11}{N11_kernel.pdf}{0.99\columnwidth}{%
        New graphic: the temporal recency kernel, weight against observation
        age.}
    \end{column}
    \begin{column}{0.37\textwidth}
      {\footnotesize Each cycle refits the parameters to the recent
      observations. The question is how far back \alert{recent} reaches.

      \vspace{0.5em}
      Everything inside a flat horizon counts fully, and older observations
      fade on a \alert{power-law tail}, never truncated. Four-year-old data
      still carries a fifth of full weight.}
      \vspace{0.6em}

      {\scriptsize One exponent sets the trade-off: track change quickly, or
      hold on to history. It is set per observation channel.}
      \vspace{0.5em}

      {\scriptsize The cycle can also run \alert{streaming MCMC}, carrying a
      full posterior forward instead of a point estimate.}
    \end{column}
  \end{columns}
\end{frame}
